---
layout: post
title: "【2026 米蘭到策馬特攻略】：瑞士義大利跨境買票精省教學"
date: 2026-04-12
categories: 策馬特
image: /img/zermatt/milano_cover.png
thumbnail: /img/zermatt/milano_cover.png
og_image: /img/zermatt/milano_cover.png
description: "直飛義大利米蘭之後想去策馬特要怎麼買票？有瑞士的 Swiss Travel Pass 或半價卡要怎麼買這段票，一篇帶你看懂（內附試算工具）。"
permalink: /blog/zermatt-milano-how-to/
---

## 怎麼從米蘭到策馬特？

先說結論，沒有直達的火車，都要轉車而且最快也要四小時。大家可能都從台北直飛米蘭 (MXP) 後，先到米蘭觀光幾日，再前往策馬特。但這條路線跨越瑞義國境，購票複雜，容易花冤望錢。這篇文章幫你整理好最關鍵的訂票省錢秘訣。如果有瑞士的 Swiss Travel Pass 或半價卡要怎麼買這段票。

<figure>
  <img src="{{ '/img/zermatt/milano_zermatt_map.png' | relative_url }}" alt="米蘭到策馬特地圖">
  <figcaption>米蘭到策馬特最快路線 經 Domodossola</figcaption>
</figure>

## 關鍵就是義大利 Domodossola 這站

從米蘭到策馬特最快的車約可以在四小時左右抵達，轉車一次 (在瑞士 Brig 或 Visp 轉車)，不過這路線班次不多，建議先用 SBB App 查好班次，不然都要花上五小時以上。

有半價卡或 Swiss Travel Pass (STP) 的旅客，從 Domodossola 之後就可以享有折扣，也就是說有 STP 的你只需要買票買到 Domodossola，之後免費，而有半價卡的 Domodossola 之後到策馬特半價。義大利境內車票比瑞士便宜很多，這段米蘭到 Domodossola 票價約 16 歐（在[義大利國鐵 Trenitalia 官網](https://www.trenitalia.com/en.html)買。）

{% assign target_post = site.posts | where: "url", "/blog/stp-halbtax-guide/" | first %}
什麼是半價卡？什麼是 STP？請看這篇
<a href="{{ target_post.url }}" class="swiss-red-link">{{ target_post.title }}</a>

<div class="tip-box" markdown="1">
### 💡 懶人包
- 有 STP 的人：買米蘭到 Domodossola 就好，後段免費（義大利國鐵官網買），約 16 歐
- 有半價卡的人：買米蘭到策馬特（用 SBB App 買選有半價卡），約 50 - 70 瑞郎（跟時間有關）
</div>

有些車沒有直達到瑞士，就需要在 Domodossola 多換一次車，此站偏荒涼。

<figure style="max-width: 400px; margin: 20px auto;">
  <img src="{{ '/img/zermatt/zermatt_milano_sbb.png' | relative_url }}" alt="米蘭到策馬特轉車時間">
  <figcaption>米蘭到策馬特最快路線詳細時間表，圖上顯示票價約要 80 瑞郎，持 STP 者等於只要花 16 歐買義大利部分的票就好。</figcaption>
</figure>

## 小提醒 義大利與瑞士火車

米蘭中央車站有閘門，進站時需刷票（可存在手機），但瑞士境內大多沒有閘門，上車就好，通常會有人來查票，給他看票即可。米蘭中央車站月台會開車前幾分鐘才顯示，不知為何有點不方便，不過可以先刷票進去裡面月台等，可能稍微安全一點。另外瑞士的火車沒有訂位，有空位就可以坐。在瑞士轉車時間通常都夠，就算只有五分鐘，除非你行動不便或行李很大件，可能就搭下一班。

過了 Domodossola 之後會進入長約 20 公里的 Simplon 隧道，出隧道你就到瑞士了。過了 Domodossola 但進隧道之前風景不錯，可以看到阿爾卑斯山深山中的義大利小村莊，建議可以先別睡覺。順帶一提這個隧道差點在二戰時被炸掉，還好最後沒有。在 Domodossola 站時可能會有邊境警察上來檢查護照或海關來看你有沒有帶什麼違禁品，所以會停個一陣子才開車。


{% assign target_post = site.posts | where: "url", "/blog/zermatt-pass-guide/" | first %}
到了策馬特之後，可以參考這篇
<a href="{{ target_post.url }}" class="swiss-red-link">{{ target_post.title }}</a>


## 💡 還是不知道要買啥票？
直接去 **[瑞士票券精算器](https://calculator.swisspro.tw)** 輸入你的行程及日期，我們會自動幫你比對各種方案的價差，看買什麼最划算。或者直接去 **[IG](https://www.instagram.com/swisspro.tw/)** 私訊發問。
